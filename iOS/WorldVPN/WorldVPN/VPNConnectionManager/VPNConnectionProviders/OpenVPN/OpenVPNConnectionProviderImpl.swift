//
//  OpenVPNConnectionProviderImpl.swift
//  WorldVPN
//
//  Created by Mustafa on 10.10.2025.
//

import Foundation
import NetworkExtension

final class OpenVPNConnectionProviderImpl: ConnectionProtocolProvider {
    private let providerManager: NETunnelProviderManager = NETunnelProviderManager()
    private let configFactory: VPNProtocolConfigFactory = VPNProtocolConfigFactory()

    func connect(server: Server) throws {
        providerManager.loadFromPreferences { loadError in
            guard loadError == nil else {
                return
            }

            let config = self.configFactory.getOpenVPNConfig(server: server)
            self.providerManager.protocolConfiguration = config
            self.providerManager.localizedDescription = "WorldVPN"
            self.providerManager.isEnabled = true

            self.providerManager.saveToPreferences { saveError in
                guard saveError == nil else {
                    return
                }

                do {
                    try self.providerManager.connection.startVPNTunnel()
                }
                catch {
                    print(error.localizedDescription)
                }
            }

        }
    }
    
    func disconnect() throws {}
}

let cert = """
client
dev tun
remote tr1.ipcover.net 20 tcp
auth-user-pass
auth-nocache
nobind
auth SHA256
cipher AES-256-GCM
tls-client
tls-cipher TLS-ECDHE-ECDSA-WITH-AES-256-GCM-SHA384
remote-cert-eku "TLS Web Server Authentication"
persist-key
persist-tun
data-ciphers AES-256-GCM

<tls-crypt>
#
# 2048 bit OpenVPN static key
#
-----BEGIN OpenVPN Static key V1-----
5a406c73bc41f6765ee9f7e082155b66
c7cff844c57e04528166f3b5b3c20330
36ab9a3ef4b8734b938e1e2650be9656
8ec1b72cf32bbe67cc3cf6b87be007b4
10f49fae4266bdbb23925edc725995bc
eaebd017b072738dac55c3309c735a43
7b05c64f4ffcb214ecb4b5ed470f768c
e667f3f678c5da89be34917be1324fde
d515c0ca61762b8a60846ae8939452ec
c9b0dd4bc28df79e8dd9ac975f9098c3
7140c326deee3ed61ce5402cc2ae0293
3a56f68ac2b1148ba661af4970f2c654
791e7cbbf13da7b5819355bd6f43e1b9
93cafbf035d4a187bc9ee3175e706563
3f5ae8f6356b3d33f18dd831235a03d7
a6eea38085fed6927e9a604b82c3ccc1
-----END OpenVPN Static key V1-----
</tls-crypt>

<ca>
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
</ca>
""".data(using: .utf8)!

